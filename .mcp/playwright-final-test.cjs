...
(async () => {
    ...
    // DOMContentLoaded 완료 대기
    await page.waitForLoadState('domcontentloaded');
    
    // 추가로 1초 대기 (이벤트 리스너 등록 보장)
    ...
    try {
        ...
        // 모달 표시 확인
        await page.waitForSelector('#imageModal', { state: 'visible', timeout: 2000 });
        ...
        // 성공 판정
        if (imageClickDuration < 200 && closeClickDuration < 200) {
            console.log('\n성공! 목표 200ms 이내 달성!');
            console.log('이미지 클릭: ' + imageClickDuration + 'ms');
            console.log('닫기 클릭: ' + closeClickDuration + 'ms');
        } else if (imageClickDuration < 500 && closeClickDuration < 500) {
            console.log('\n양호! 500ms 이내 달성');
            console.log('이미지 클릭: ' + imageClickDuration + 'ms');
            console.log('닫기 클릭: ' + closeClickDuration + 'ms');
        }
