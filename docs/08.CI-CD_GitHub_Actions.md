...
> **⚠️ 중요**: 이 문서는 실제 운영 중인 시스템을 설명합니다. 변경 시 신중히 검토하세요.
...
#### 방식 1: Auto-push 시스템 (현재 운영 중)
1. **Claude Code**: 코드 수정 → 커밋 → 태그 생성
2. **Crontab**: 정기적으로 로컬 저장소 확인 → GitHub 자동 푸시 (15분 간격)
3. **GitHub Actions**: 푸시 감지 → 자동 배포 트리거
4. **배포 시스템**: 테스트 → 스테이징 → 프로덕션 배포

#### 방식 2: Claude Code 직접 푸시 (선택 가능)
1. **Claude Code**: 코드 수정 → 커밋 → **직접 GitHub 푸시**
2. **GitHub Actions**: 푸시 감지 → 자동 배포 트리거
3. **배포 시스템**: 테스트 → 스테이징 → 프로덕션 배포

> **💡 Tip**: Claude Code가 직접 푸시하면 15분 대기 없이 즉시 배포 가능
...
- **로그 로테이션**: 로그파일이 10MB 초과 시 자동 로테이션 (⚠️ 현재 7.1MB, 수동 정리 권장)
- **에러 처리**: 실패 시 상세한 에러 정보 로깅

> **⚠️ 로그 관리 필요**: 현재 로그 파일이 7.1MB로 증가했습니다. 태그 충돌로 인한 반복 에러가 누적되고 있습니다.
...
**태그 충돌 (현재 발생 중):**
```
2025-10-18 17:15:04 - INFO: Local repository is 213 commits ahead. Pushing to GitHub...
2025-10-18 17:15:05 - ERROR: Failed to push to GitHub
2025-10-18 17:15:05 - ERROR_DETAILS: To https://github.com/LeeJeongHyeon123/topmkt.git
 ! [rejected]          v3.89.7 -> v3.89.7 (already exists)
error: failed to push some refs to 'https://github.com/LeeJeongHyeon123/topmkt.git'
...
- `feature/*`: 기능 개발 브랜치, PR 후 `develop`으로 병합
  - 예: `feature/refactoring-v4.0.0` (현재 주요 개발 브랜치, v3.89.7 진행 중)
- `hotfix/*`: 긴급 수정 브랜치, PR 후 `master`로 직접 병합 가능

> **📌 현재 상태 (v3.89.7)**: `feature/refactoring-v4.0.0` 브랜치에서 개발 진행 중
...
#### 문제 1: 태그 충돌 (Already Exists) ⚠️ **실제 발생 중**
**증상**: 로그에 `! [rejected] vX.X.X -> vX.X.X (already exists)` 에러
**원인**: 이미 원격 저장소에 존재하는 태그를 다시 푸시하려고 시도
**해결**:
```bash
# 방법 1: 태그 제외하고 푸시 (권장)
cd /var/www/html/topmkt
git push origin feature/refactoring-v4.0.0  # 브랜치만 푸시

# 방법 2: 태그 강제 업데이트 (신중히 사용)
git push origin --tags --force

# 방법 3: 특정 태그 삭제 후 재생성
git tag -d v3.89.7                    # 로컬 태그 삭제
git push origin :refs/tags/v3.89.7    # 원격 태그 삭제
git tag v3.89.7                        # 새 태그 생성
git push origin v3.89.7                # 푸시
```

**예방**:
- Auto-push 스크립트 수정하여 태그 푸시 제외
- 또는 태그는 수동으로만 관리
...
cd /var/www/html/topmkt
git config credential.helper store
git push origin feature/refactoring-v4.0.0
```
...
# 로그 파일 크기 확인 (현재 7.1MB - 로테이션 필요)
ls -lh /var/log/auto-git-push.log
```
...
- **로그파일 관리 중요**: 현재 7.1MB까지 증가 (정기적 정리 필수)
- crontab 설정 후 반드시 동작 테스트 필요
- **태그 충돌 주의**: 동일 태그를 여러 번 푸시하면 에러 발생 (현재 발생 중)
...
### ⚡ 개발 속도 향상:
- **Claude Code**: 코드 작성 + 커밋만 담당 (또는 직접 푸시)
- **자동 푸시**: 15분 내 GitHub 자동 반영 (또는 즉시 푸시)
- **자동 배포**: GitHub Actions 트리거로 즉시 배포
- **무중단 개발**: 수동 작업 없이 연속 개발 가능

### 🎯 자동화 워크플로우:

**방식 1: Auto-push 시스템 (현재 운영)**
```
...
**방식 2: Claude Code 직접 푸시 (선택 가능)**
```
Claude Code 작업 → 커밋 + 푸시 → GitHub Actions 트리거 → 자동 배포 → 완료! (15분 단축)
```

> **💡 추천**: 긴급 배포가 필요한 경우 방식 2 사용, 일반 개발은 방식 1 사용
