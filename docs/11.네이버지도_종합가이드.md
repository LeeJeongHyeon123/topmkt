...
> 💡 **빠른 참조**: 네이버 Maps API 기본 설정만 필요하신가요?
>
> [**10.외부서비스_연동가이드.md**](10.외부서비스_연동가이드.md) - 6개 외부 서비스 통합 가이드
> - 빠른 설정 방법 (5분 이내)
> - 다른 외부 서비스와 함께 비교
> - 기본 사용 예제
>
...
define('NAVER_MAPS_CLIENT_SECRET', '[발급받은_클라이언트_시크릿]');    // ⚠️ 실제 시크릿으로 변경
```

> 🔥 **보안 중요**:
> - 클라이언트 ID와 시크릿은 절대 GitHub이나 공개 문서에 포함하지 마세요!
> - 환경 변수 또는 별도의 설정 파일로 관리 권장
> - `.gitignore`에 설정 파일 추가 필수

**중요**: 네이버 클라우드 플랫폼에서 발급받은 실제 API 키를 서버 설정 파일에만 저장하세요
...
> ⚠️ **보안**: 실제 Client ID는 문서에 기재하지 않습니다.
...
> 🔐 **보안 최우선**: API 키 노출은 심각한 보안 사고를 초래할 수 있습니다!

### 🔒 6.1 API 키 보안 관리

#### 클라이언트 ID 보안
- **도메인 제한 설정 필수**: NCP 콘솔에서 정확한 도메인만 등록
- **정기적인 키 교체**: 최소 6개월마다 재발급 권장
- **사용량 모니터링**: 비정상 사용 패턴 즉시 탐지
- **GitHub 업로드 금지**: 절대 공개 저장소에 포함하지 마세요!

#### 클라이언트 시크릿 보안
- **서버 사이드 전용**: 클라이언트(브라우저)에 절대 노출 금지
- **환경 변수 사용**: `.env` 파일 또는 서버 환경 변수로 관리
- **설정 파일 보호**: `config.php`를 `.gitignore`에 추가
- **로그 주의**: 로그 파일에 시크릿 출력 금지

#### 보안 체크리스트
- [ ] `.gitignore`에 `src/config/config.php` 추가됨
- [ ] API 키가 코드에 하드코딩되지 않음
- [ ] NCP 콘솔에서 도메인 제한 설정 완료
- [ ] 사용량 알림 설정 완료 (무료 한도 80% 도달 시)
- [ ] 로그에 API 키/시크릿 노출 없음 확인
...
> 📚 **관련 문서**: [10.외부서비스_연동가이드.md](10.외부서비스_연동가이드.md) - 전체 외부 서비스 통합 가이드 